...
Route::get('/tag/sync', function () {
    $post = Post::find(1);
    $post->tags()->sync([3,4], true);
    // Data yang duplikasi seperti 5,6,7 akan digantikan id 3 dan 4 
    // sync() sama seperti melakukan detach() dan attach() secara bersamaan
    // Parameter 2 yaitu : 
    // true : menghapus data duplikasi dan mengganti sesuai data sync
    // false : tidak menghapus data duplikasi namun menambahkan data baru dari sync

    $portfolio = Portfolio::find(1);
    $portfolio->tags()->sync([3], false);
    // Method sync() harus bertipe array [] seperti sync([])
